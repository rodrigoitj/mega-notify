...
function Events(props) {
  ...
  function calcPercentage(
    msRemainingToEvent,
    secondsToEventFromStart,
  ) {
    // {secs / Math.round(props.total / 100, 0) -
    //   1}
    // - {Math.round(props.total / 1000, 0)} -{" "}
    // {secs};
    const secondsRemainingToEvent = Math.round(
      msRemainingToEvent / 1000,
      0,
    );
    const secondsPassed =
      secondsToEventFromStart - secondsRemainingToEvent;
  ...
  useEffect(() => {
    load('https://megamu.net/events.js', function (err) {
      if (err) {
        ...
        var builtEventsSchedules = buildEventSchedules(
          window.eventsTime,
        );
